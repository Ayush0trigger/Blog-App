...
export const blogPost = catchAsyncErrors(async (req, res, next) => {
   ...
   if (!allowedFormats.includes(mainImage.mimetype) ||
      ...
      return next(new ErrorHandler("Invalid Image Format!. Only JPEG, PNG, WEBP formats are allowed", 400));
   }
   ...
   const uploadPromises = [
      ...
      // cloudinary.uploader.upload(path.resolve(mainImage.tempFilePath)),
   ...
   if (!mainImageRes || mainImageRes.error || (paraOneImage && (!paraOneImageRes || paraOneImageRes.error)) || (paraTwoImage && (!paraTwoImageRes || paraTwoImageRes.error)) ||
...
export const updateBlog = catchAsyncErrors(async (req, res, next) => {
   ...
   if(req.files){
      ...
      if(req.files && mainImage){
         ...
         await cloudinary.v2.uploader.destroy(blogMainImageId);  
...
if(req.files && paraOneImage){
   ...
   newBlogData.paraOneImage = {public_id : newBlogParaOneImage.public_id,
...
if(req.files && paraTwoImage){
   ...
   newBlogData.paraTwoImage = {public_id : newBlogParaTwoImage.public_id,
...
if(req.files && paraThreeImage){
   ...
   newBlogData.paraThreeImage = {public_id : newBlogParaThreeImage.public_id,
...
export const dislikeBlog = catchAsyncErrors(async (req, res, next) => {
    const { id } = req.params;
    const userId = req.user._id;
...
export const getBlogsByAuthor = async (req, res, next) => {
  try {
    ...
    // Get all blogs by this author using createdBy field
    const blogs = await Blog.find({ createdBy: authorId, published: true })
      .sort({ createdAt: -1 });
